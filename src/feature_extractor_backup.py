"""
feature_extractor.py - Feature Çıkarma Modülü
...
# Tanınan marka listesi (Levenshtein için)
KNOWN_BRANDS = [
...
# Homoglyph haritası (phishing'de sık kullanılan karakter değişimleri)
HOMOGLYPH_MAP = {
    'a': ['@', '4', 'а'],  # Kiril 'а'
    'e': ['3', 'е'],       # Kiril 'е'
    'i': ['1', 'l', '!', 'і'],
...
class URLFeatureExtractor:
    """URL string üzerinden istatistiksel feature çıkarır."""

    def extract(self, url: str) -> Dict:
        """Tek URL için tüm URL feature'larını çıkarır."""
        features = {}
        ...
        try:
            ...
            # ── Digit / Alpha / Özel Karakter Oranı ──────
    ...
    def _zero_features(self) -> Dict:
        """Hata durumunda sıfır feature dict döner."""
        return {f: 0 for f in self._get_feature_names()}

    def _get_feature_names(self) -> List[str]:
        """Feature isimlerini döner."""
...
class AdversarialFeatureExtractor:
    """
    Phishing URL'lerin marka taklidi yapıp yapmadığını tespit eder.
    Levenshtein distance + Homoglyph detection
    """

    def extract(self, url: str) -> Dict:
        """Adversarial feature'ları çıkarır."""
        features = {}
        ...
        try:
            ...
            # ── Brand İçinde mi? ──────────────────────────
            # Domain içinde marka ismi geçiyor ama domain marka değil
    ...
    def _levenshtein_all_brands(self, domain: str) -> Dict:
        """Tüm markalar arasında minimum Levenshtein'ı bulur."""
        min_dist    = float('inf')
        closest     = "none"
    ...
    def _detect_homoglyph(self, domain: str) -> int:
        """Domain'de homoglyph karakteri var mı?"""
    ...
    def _count_homoglyphs(self, domain: str) -> int:
        """Domain'deki toplam homoglyph sayısını sayar."""
        count = 0
        for char, glyphs in HOMOGLYPH_MAP.items():
            for g in glyphs:
                if g in domain:
    ...
    def _brand_in_domain(self, domain: str) -> int:
        """
        Marka ismi domain içinde geçiyor ama domain markadan farklı.
        Örneğin: 'paypal-secure.com' → paypal geçiyor ama paypal.com değil
        """
        for brand in KNOWN_BRANDS:
    ...
    def _has_digit_substitution(self, domain: str) -> int:
        """
        Harf yerine rakam kullanımı var mı?
        Örneğin: paypa1 (1 yerine l), g00gle (00 yerine oo)
        """
        substitutions = [
            ("0", "o"), ("1", "l"), ("1", "i"),
            ("3", "e"), ("4", "a"), ("5", "s"),
        ...
        for digit, letter in substitutions:
            if digit in domain:
                # Bu digit kaldırılırsa bilinen bir markaya benziyor mu?
    ...
    def _count_repeated_chars(self, domain: str) -> int:
        """Ard arda tekrar eden karakter sayısını döner (gooogle gibi)."""
        count = 0
    ...
    def _confusion_score(self, domain: str, lev_scores: Dict) -> float:
        """
        0-1 arası genel karışıklık skoru.
        Levenshtein, homoglyph ve brand-in-domain birleşimi.
        """
        score = 0.0
        # Yakın Levenshtein → yüksek skor
...
# ─────────────────────────────────────────────
# METADATA FEATURE PROCESSOR
# ─────────────────────────────────────────────

class MetadataFeatureProcessor:
    """
    Ham WHOIS/SSL metadata'yı ML-ready feature'lara dönüştürür.
    """

    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        """Metadata sütunlarını işler ve yeni feature'lar türetir."""
        df = df.copy()
        
        # Domain yaşı feature'ları
...
# ─────────────────────────────────────────────
# ANA FEATURE PIPELINE
# ─────────────────────────────────────────────

class FeaturePipeline:
    """Tüm feature extractor'ları birleştiren ana sınıf."""
    ...
    def transform(self, df: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
        """
        DataFrame'deki her URL için tüm feature'ları çıkarır.
        
        Args:
            df: 'url' sütunu içeren DataFrame
        Returns:
        ...
        # 1. URL Features
        print("   [1/3] URL features çıkarılıyor...")
        url_features = []
        ...
        print("   [2/3] Adversarial features çıkarılıyor...")
        adv_features = []
        ...
        # 3. Metadata features işle
        print("   [3/3] Metadata features işleniyor...")
        meta_cols = [
        ...
        # timestamp varsa ekle (zaman bazlı split için)
        if "timestamp" in df.columns:
            feature_df["timestamp"] = df["timestamp"].values
        
        # Kategorik sütunları kaldır
        drop_cols = ["closest_brand"]  # string sütun
        ...
        print(f"✅ Feature extraction tamamlandı: {len(feature_df)} satır, {len(feature_cols)} feature")
        return feature_df

    def get_feature_names(self) -> List[str]:
        """Feature isimlerini döner (url, label, timestamp hariç)."""
...
def time_based_split(df: pd.DataFrame,
    ...
    """
    Zero-day odaklı zaman bazlı train/test split.
    
    2023 ve öncesi → TRAIN
    2024 ve sonrası → TEST
    
    Args:
        df: timestamp sütunu olan feature DataFrame
    Returns:
    ...
    if len(test_df) == 0:
        print("⚠️  Test seti boş! Timestamp dağılımı kontrol ediliyor...")
        print(df["timestamp"].dt.year.value_counts())
        # Fallback: son %20
        split_idx = int(len(df) * 0.8)
        train_df  = df.iloc[:split_idx]
        test_df   = df.iloc[split_idx:]
        print("   Fallback: son %20 test seti olarak kullanıldı")
    ...
    print(f"\n📅 Zaman Bazlı Split:")
    print(f"   TRAIN: {len(train_df):,} örnek (≤ {train_end_year})")
    print(f"   TEST:  {len(test_df):,} örnek (≥ {test_start_year})")
    print(f"   Feature sayısı: {len(feature_cols)}")
    print(f"   Train Phishing: {y_train.sum()} | Legitimate: {(y_train==0).sum()}")
    print(f"   Test  Phishing: {y_test.sum()} | Legitimate: {(y_test==0).sum()}")
...
if __name__ == "__main__":
    ...
    test_urls = [
        ...
        "https://faceb00k-support.com/account/suspended",
        "https://www.github.com/user/repo",
    ]
    test_df = pd.DataFrame({
        "url": test_urls,
        "label": [1, 0, 1, 1, 0],
        "timestamp": pd.date_range("2023-01-01", periods=5, freq="6ME", tz="UTC")
    })
